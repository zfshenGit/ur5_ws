...
void UrRealtimeCommunication::setSpeedl(double q0, double q1, double q2,
		double q3, double q4, double q5, double acc) {
	char cmd[1024];
	if( robot_state_->getVersion() >= 3.3 ) {
		sprintf(cmd,
		...
		sprintf(cmd,
				"speedl([%1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f], %f)\n",
				q0, q1, q2, q3, q4, q5, acc);
	}
	else {
		sprintf(cmd,
				"speedl([%1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f], %f, 0.02)\n",
				q0, q1, q2, q3, q4, q5, acc);	
	}
	addCommandToQueue((std::string) (cmd));
	if (q0 != 0. or q1 != 0. or q2 != 0. or q3 != 0. or q4 != 0. or q5 != 0.) {
...
void UrRealtimeCommunication::setMovel(double q0, double q1, double q2,
		double q3, double q4, double q5, double acc,double vel) {
	char cmd[1024];
	if( robot_state_->getVersion() >= 3.3 ) {
		sprintf(cmd,
				"movel([%1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f], %f, %f, 0, 0)\n",
				q0, q1, q2, q3, q4, q5, acc,vel);
	}
	else if( robot_state_->getVersion() >= 3.1 ) {
		sprintf(cmd,
				"movel([%1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f], %f ,%f)\n",
				q0, q1, q2, q3, q4, q5, acc, vel);
	}
	else {
		sprintf(cmd,
				"movel([%1.5f, %1.5f, %1.5f, %1.5f, %1.5f, %1.5f], %f, %f, 0)\n",
				q0, q1, q2, q3, q4, q5, acc ,vel);	
	}
	if(acc<0){
		sprintf(cmd,"stopl(3)\n");
