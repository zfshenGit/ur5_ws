...
public:
	RosWrapper(std::string host, int reverse_port) :
		...
		if (robot_.start()) {
			...
			position_sub_ = nh_.subscribe("ur_driver/tcp_position", 1,
					&RosWrapper::positionInterface, this);
...
/*
	void speedInterface(const trajectory_msgs::JointTrajectory::Ptr& msg) {
		if (msg->points[0].velocities.size() == 6) {
			double acc = 0.08;//100 default
			//double acc = msg->points[0].accelerations[0];
...
*/
	void speedInterface(const trajectory_msgs::JointTrajectory::Ptr& msg) {
		if (msg->points[0].velocities.size() == 6) {
			double acc =1.;//100 default 测试到了10,没测到30.........!!!!!
			if (msg->points[0].accelerations.size() ==1)
				acc = msg->points[0].accelerations[0];
			robot_.setSpeedl(msg->points[0].velocities[0],
					msg->points[0].velocities[1], msg->points[0].velocities[2],
					msg->points[0].velocities[3], msg->points[0].velocities[4],
					msg->points[0].velocities[5], acc);
		}
	...
	void positionInterface(const trajectory_msgs::JointTrajectory::Ptr& msg){
		if(msg->points[0].positions.size()==6){
			double vel =0.25;
			double acc = 1.4;
			if(msg->points[0].velocities.size()==1){
				vel =msg->points[0].velocities[0];
			}
			if(msg->points[0].accelerations.size()==1){
				acc = msg->points[0].accelerations[0];
			}
			robot_.setMovel(msg->points[0].positions[0],
							msg->points[0].positions[1],msg->points[0].positions[2],
							msg->points[0].positions[3],msg->points[0].positions[4],
							msg->points[0].positions[5],acc,vel);
		}
	}
